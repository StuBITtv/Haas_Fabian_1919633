...
START_TEST(only_positiv_real_part) {
    stringTest(getComplex(1.2, 0), 1.2, M_PI, "%r*(cos(%t)+i*sin(%t)");
}

END_TEST

START_TEST(only_negativ_real_part) {
    stringTest(getComplex(-1.2, 0), 1.2, -1 * M_PI, "%r*(cos(%t)+i*sin(%t)");
}
...
START_TEST(only_negativ_imaginary_part) {
    stringTest(getComplex(0, -3.4), 3.4, -1 * M_PI / 2, "%r*(cos(%t)+i*sin(%t)");
}
